...
const index_js_2 = require("../graph/index.js");
const index_js_3 = require("../manifest/index.js");
const index_js_4 = require("../cache/index.js");
const index_js_5 = require("../codegen/index.js");
const index_js_6 = require("../runner/index.js");
const index_js_7 = require("../writer/index.js");
const fix_js_1 = require("./fix.js");
...
program
    ...
    const parser = new index_js_1.PxmlParser();
    const project = parser.parse(projectXml);
    const graph = new index_js_2.DependencyGraph(project.nodes);
    const order = graph.getSortOrder();
    const manifest = new index_js_3.PxmlManifest(cwd, project.name, project.version);
    const writer = new index_js_7.FileWriter(!!options.dryRun);
    const apiKey = options.apiKey || (options.provider === 'openai' ? process.env.OPENAI_API_KEY : process.env.ANTHROPIC_API_KEY);
    const codegen = new index_js_5.PxmlCodegen({
    ...
    for (const nodeId of order) {
        ...
        const xmlHash = index_js_4.PxmlCache.hashNode(node);
...
program
    ...
    const project = parser.parse(projectXml);
    const manifest = new index_js_3.PxmlManifest(cwd, project.name, project.version);
    const writer = new index_js_7.FileWriter();
    const runner = new index_js_6.PxmlRunner(cwd, writer);
    let allPassed = true;
...
program
    ...
    for (const node of targetNodes) {
        ...
        if (!testRes.passed) {
            const success = await (0, fix_js_1.runFixLoop)(node, cwd, manifest, codegen, runner, writer);
            if (success) {
